...
--@ BeginMethod
--@ MethodExecSpace=All
number MathExpressionEvalWithProperty(any actor,string s)
{
--先將字串替換成對應角色屬性值
--遊戲目前影響技能傷害的屬性列表如下:
--str,dex,vit,int,agi,luk,atk,def,maxHp,maxMP
s = s:lower()

if s:find("str") ~= nil then
	s = s:gsub("str",actor.str)
end

if s:find("agi") ~= nil then
	s = s:gsub("agi",actor.agi)
end

if s:find("vit") ~= nil then
	s = s:gsub("vit",actor.vit)
end
